impl Solution {
    pub fn add_binary(a: String, b: String) -> String {
        let valor = | c | if c == '0' { 0 } else { 1 };
        let mut a: Vec<i32> = a.chars ().map (| c | valor (c)).collect (); // Sombreia
        let mut b: Vec<i32> = b.chars ().map (| c | valor (c)).collect (); // Idem
        a.reverse ();
        b.reverse ();
        ...
        String::from_iter (
            resposta
                .into_iter ()
                .rev ()
                .map (| v | (v + b'0' as i32) as u8 as char)
        )
    }
}
