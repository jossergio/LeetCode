func addToArrayForm(num []int, k int) []int {
    ...
    for i := len (num) - 1; i >= 0; i -= 1 {
        ...
        resposta = append (resposta, adicao)
    }
    if vaiUm == 1 { // Sobrou
        resposta = append (resposta, 1)
    }
    return inverter (resposta) // Vem com os dígitos invertidos
}
