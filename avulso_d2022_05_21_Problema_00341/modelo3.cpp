...
private:
    ...
    void montar (vector<NestedInteger> &n) {
        ...
        pilha.push ({n.begin (), n.end ()});
        
        while (! pilha.empty ()) {
            vector<NestedInteger>::iterator p = pilha.top ().first;
            if (p != pilha.top ().second) {
                NestedInteger item = *p;
                pilha.top ().first++;
                if (item.isInteger ()) {
                    valores.push_back (item.getInteger ());
                    cout << "Colocou " << valores.back () << endl;
                } else {
                    vector<NestedInteger>* tmp = new vector<NestedInteger> (item.getList ());
                    pilha.push ({tmp->begin (), tmp->end ()});
                }
            } else {
                pilha.pop ();
            }
