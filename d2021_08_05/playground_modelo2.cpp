...
// Por conta de evitar passagens por referências e outros detalhes, fiz uma função para cada um    
    void somarAlex (int valor, int op) { // op é adicionar ou subtrair (+1 ou -1)
        somaAlex += op * valor;
        soma -= op * valor; // O contrário. Se soma a um, tira do outro
    } // somarAlex
    
    void somarLee (int valor, int op) { // Idem Alx
        somaLee += op * valor;
        soma -= op * valor;
    } // somarLee
#ifdef DEBUG    
    ...
    bool teste () {
#endif
        if (somaAlex > somaLee + soma) // Alex ganha, independente de outras iterações
            return true;
#ifdef DEBUG
...
#endif
#ifdef DEBUG
// Como há vários detalhes em debug, juntei tudo
// Ficou repetitivo, mas, ...
        chamada++;
        if (f > i) {
// Alex e Lee tiram do mesmo lado, esquerdo
            somarAlex (temp [i++], 1);
            somarLee (temp [i++], 1);
cout << "<" << setw (8) << chamada << "> " << iteracao << " (linha 1) | " << i << " " << f;
cout << " ** " << soma << " " << somaAlex << " " << somaLee << endl;
            if (teste (iteracao)) // Alex já ganha
                return true; // É que se for falso, ainda não terminou
// Retorna Lee para tirar do lado direito            
            somarLee (temp [--i], -1);
            somarLee (temp [f--], 1);
cout << "<" << setw (8) << chamada << "> " << iteracao << " (linha 2) | " << i << " " << f;
cout << " ** " << soma << " " << somaAlex << " " << somaLee << endl;
            if (teste (iteracao))
                return true;
// Volta tudo e Alex e Lee começam tirando do lado direito
            ...
            somarAlex (temp [f--], 1);
            somarLee (temp [f--], 1);
cout << "<" << setw (8) << chamada << "> " << iteracao << " (linha 3) | " << i << " " << f;
cout << " ** " << soma << " " << somaAlex << " " << somaLee << endl;
            if (teste (iteracao))
                return true;
// Retorna Lee para tirar do início
            somarLee (temp [++f], -1);
            somarLee (temp [i++], 1);
cout << "<" << setw (8) << chamada << "> " << iteracao << " (linha 4) | " << i << " " << f;
cout << " ** " << soma << " " << somaAlex << " " << somaLee << endl;
            if (teste (iteracao))
                return true;
            somarLee (temp [--i], -1);
